...
import org.bachelor.userlogin.service.impl.PupUserServiceImpl;
...
public class UserController {
  ...
  @Autowired
  private PupUserServiceImpl pupUserService;
  ...
  public ResponseEntity<JsonResponse> accesstoken(@RequestParam String code, HttpServletRequest request,
    ...
    Date expTime;
    try {
      expTime = sdf.parse(upCC.getExpiresTime());
    ...
    userObject.put("iat", new Date().getTime());
  ...
  private JSONArray getRoleList(String accessToken,String userId) {
    SignSecurityOAuthClient oAuthClient = new SignSecurityOAuthClient(new URLConnectionClient());
    OAuthResourceClientRequest userRolesRequest =
            new OAuthResourceClientRequest("userRoles", userSysConfig.getUseRoleUrl(), OAuth.HttpMethod.GET);
    userRolesRequest.setAccessToken(accessToken);


    userRolesRequest.setParameter("clientId", userSysConfig.getClientId());
    userRolesRequest.setParameter("userId",userId);
    OAuthResourceResponse resourceResponse = null;
    try {
      resourceResponse = oAuthClient.resource(userRolesRequest, OAuthResourceResponse.class);
    } catch (Exception e) {
      logger.error(e.getMessage());
      throw new SystemException(e.getCause());
    }
    String roleInfoRes = resourceResponse.getBody();
    // logger.info(orgInfoRes);
    JSONObject userObject = JSONObject.fromObject(roleInfoRes);
    JSONArray roleArray = userObject.getJSONArray("rows");
    return roleArray;
  }
  ...
  public ResponseEntity<JsonResponse> logout(@RequestHeader("Authorization") String token) {
    ...
    pupUserService.logout(userCode);
    return JsonResponse.createHttpEntity("logout success",HttpStatus.OK);
  }

  @RequestMapping(value = "/pupsecret", method = RequestMethod.GET)
  public HttpEntity<JsonResponse> pupConfig() {
    return JsonResponse.createHttpEntity(userSysConfig.getClientSecret(),HttpStatus.OK);
  }
